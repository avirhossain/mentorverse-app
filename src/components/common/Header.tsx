...
export const Header = ({ isMenuOpen, setIsMenuOpen, currentView }) => {
    ...
    <header className="sticky top-0 z-50 bg-white shadow-lg border-b border-primary/10">
        <div className="max-w-7xl mx-auto p-4 flex justify-between items-center">
            <Link href={isAdminView ? "/admin" : "/home"} className="text-2xl font-extrabold text-primary">Guidelab</Link>
            ...
            <nav className="hidden lg:flex space-x-2 items-center text-gray-600 font-medium">
                {!isAdminView && (
                 ...
                 {isAdminView && (
                     <Link href="/admin" className={`flex items-center transition px-3 py-2 rounded-lg text-white bg-primary font-bold`}>
                        <Shield className="w-5 h-5 mr-1" />Admin Dashboard
                    </Link>
                 )}
            </nav>
