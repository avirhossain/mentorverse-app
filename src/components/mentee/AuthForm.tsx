...
export function AuthForm() {
  ...
  const handleAuthAction = async (action: (auth: Auth, ...args: any[]) => Promise<any>) => {
    setIsLoading(true);
    try {
      const userCredential = await action(auth, email, password);
      if (userCredential && activeTab === 'signup') {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
        ...
        if (userCredential.user && (await MenteesAPI.getMentee(firestore, userCredential.user.uid)).exists() === false) {
          await createMenteeProfile(userCredential);
        }
         toast({
  ...
  return (
    <Card className="w-full max-w-md mx-4">
      <CardHeader className="text-center">
        <CardTitle className="text-3xl font-bold text-primary">MentorVerse</CardTitle>
        <CardDescription>
          Your gateway to expert mentorship.
        </CardDescription>
      </CardHeader>
      <CardContent>
          ...
          <TabsContent value="login">
            <form onSubmit={handleEmailPasswordSubmit} className="space-y-4 pt-4">
              <div className="space-y-2">
                ...
                <Input id="login-email" type="email" placeholder="m@example.com" required value={email} onChange={e => setEmail(e.target.value)} disabled={isLoading} />
              </div>
              <div className="space-y-2">
          ...
          <TabsContent value="signup">
            <form onSubmit={handleEmailPasswordSubmit} className="space-y-4 pt-4">
                <div className="space-y-2">
                    ...
                    <Input id="signup-name" type="text" placeholder="John Doe" required value={name} onChange={e => setName(e.target.value)} disabled={isLoading} />
                </div>
              <div className="space-y-2">
                <Label htmlFor="signup-email">Email</Label>
                <Input id="signup-email" type="email" placeholder="m@example.com" required value={email} onChange={e => setEmail(e.target.value)} disabled={isLoading} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="signup-password">Password</Label>
                <Input id="signup-password" type="password" required value={password} onChange={e => setPassword(e.target.value)} disabled={isLoading} />
              </div>
